...
if delta == 0: continue
movers.append({"name":p["name"],"team":p.get("team",""),"pos":p["pos"],"adp":h[-1]["adp"],"risers = sorted([m for m in movers if m["delta"]<0], key=lambda x:x["delta"])[:n]
...
return f'<span style="padding:2px 7px;border-radius:4px;font-size:10px;font-weight:700;background:{def player_row(p, is_riser, show_days=False):
arrow = "▲" if is_riser else "▼"
dc = "#4ade80" if is_riser else "#f87171"
extra = f'<div style="color:#6b6560;font-size:10px">{p.get("days",0)}d ago</div>' if show_return f"""<tr style="border-bottom:1px solid #2a2520">
...
<td style="padding:9px 8px;text-align:right;font-family:monospace;font-size:14px;font-weight:<td style="padding:9px 8px;text-align:right;font-family:monospace;font-size:13px;font-weight:</tr>"""
def section(label, color, rows_html):
if not rows_html: return ""
return f"""<div style="margin-bottom:20px">
<div style="font-size:11px;font-weight:700;color:{color};letter-spacing:0.1em;text-transform:<table style="width:100%;border-collapse:collapse;background:#1a1714;border-radius:12px;overflow:</div>"""
def divider(label):
...
<div style="font-size:10px;color:#6b6560;font-weight:700;letter-spacing:0.12em;text-transform:<div style="flex:1;height:1px;background:#2a2520"></div>
</div>"""
...
body += section(f"▲ Top {len(d_risers)} Risers", "#4ade80", "".join(player_row(p,True) body += section(f"▼ Top {len(d_fallers)} Fallers","#f87171", "".join(player_row(p,False) # ── Section 2: By position
if has_pos:
...
pc = POS_COLORS[pos]
body += f'<div style="font-size:12px;font-weight:700;color:{pc};letter-spacing:0.06em;if pr: body += section(f"▲ Risers", "#4ade80", "".join(player_row(p,True) for p if pf: body += section(f"▼ Fallers","#f87171", "".join(player_row(p,False) for p # ── Section 3: All-time
if has_alltime:
days_tracked = max((len(p.get("history",[])) for p in players), default=1)
body += divider(f" All-Time Since Day 1 ({days_tracked} days)")
body += section(f"▲ Biggest Risers", "#4ade80", "".join(player_row(p,True,True) for body += section(f"▼ Biggest Fallers","#f87171", "".join(player_row(p,False,True) for return f"""<!DOCTYPE html><html><head><meta charset="UTF-8"/></head>
<body style="margin:0;padding:0;background:#0f0d0b;font-family:'Helvetica Neue',Arial,sans-serif">
<div style="max-width:520px;margin:0 auto;padding:24px 16px">
<div style="margin-bottom:8px">
<div style="font-size:22px;font-weight:800;color:#f0ede8;letter-spacing:-0.5px">ADP<span <div style="font-size:11px;color:#6b6560;letter-spacing:0.08em;margin-top:2px">UNDERDOG FANTASY
</div>
{body}
<div style="text-align:center;margin:28px 0 20px">
<a href="{SITE_URL}" style="display:inline-block;background:#f97316;color:#0f0d0b;font-weight:</div>
<div style="font-size:11px;color:#4a4540;text-align:center;border-top:1px solid #2a2520;padding-</div></body></html>"""
