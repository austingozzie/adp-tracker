...
def calc_value_score(p):
    """
    Value = player rising on Underdog (momentum) AND priced earlier than other platforms.
    Only meaningful when BOTH Sleeper and ESPN have the player ranked.
    Score range typically -15 to +15.
    """
    h = p.get("history", [])
    adp_ud = p.get("adp_ud") or p.get("adp", 999)

    # 1. Platform discount
    # Positive = Underdog cheaper (lower ADP) than consensus = value
    # Only use when both platforms have data
    sl = p.get("adp_sleeper")
    es = p.get("adp_espn")
    if sl and es:
        consensus = (sl + es) / 2
        # Normalize: cap at +/- 30 spots difference, scale to +/- 10 pts
        raw_diff = consensus - adp_ud
        platform_score = max(-10, min(10, raw_diff / 3))
    else:
        platform_score = 0  # no data = no score

    # 2. Momentum score (7-day rolling)
    # Normalize: cap at +/- 15 spots movement, scale to +/- 10 pts
    if len(h) >= 2:
    ...
    # Equal mix, only score if we have platform data
