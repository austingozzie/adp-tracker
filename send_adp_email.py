...
def build_html(players, updated):
    ...
    if top_vals:
        ...
        for pos in ["QB","RB","WR","TE"]:
            ...
            tr = "<tr style="border-bottom:1px solid #2a2520">"
            tr += "<td style="padding:10px 8px;width:44px">"
            tr += "<span style="padding:2px 7px;border-radius:4px;font-size:10px;font-weight:700;"
            tr += "background:" + pc + "22;color:" + pc + ";font-family:monospace">" + pos + "</span>"
            tr += "</td>"
            tr += "<td style="padding:10px 8px">"
            tr += "<div style="font-weight:600;color:#f0ede8;font-size:13px">" + p["name"] + "</div>"
            tr += "<div style="color:#6b6560;font-size:11px">" + p.get("team","") + " &middot; ADP " + str(round(p.get("adp",0),1)) + "</div>"
            tr += "</td>"
            tr += "<td style="padding:10px 8px;text-align:right;font-family:monospace;font-size:14px;font-weight:700;color:#4ade80">"
            tr += "+" + str(score)
            tr += "</td></tr>"
            val_rows += tr
        val_section = "<div style="margin-bottom:20px">"
        val_section += "<div style="font-size:11px;font-weight:700;color:#fbbf24;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:8px">TOP VALUE PICKS TODAY</div>"
        val_section += "<table style="width:100%;border-collapse:collapse;background:#1a1714;border-radius:12px;overflow:hidden;border:1px solid #2a2520">"
        val_section += val_rows
        val_section += "</table>"
        val_section += "<div style="font-size:10px;color:#4a4540;margin-top:6px">Score = platform discount + momentum</div>"
        val_section += "</div>"
        body += val_section
